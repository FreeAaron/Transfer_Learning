...
def train_model(model, criterion, optimizer, scheduler, num_epochs,
                device, dataset_sizes, dataloaders):
...
if __name__ == "__main__":
    ...
    num_epochs = 2
    # 加載數據集
    dataloaders, dataset_sizes, class_names, device = get_dataset(data_dir)
    ...
    # '''
    # all backbone train
    model, criterion, optimizer, scheduler = get_full_transfer_model()
    model_ft = train_model(model, criterion, optimizer, scheduler, num_epochs,
                           device, dataset_sizes, dataloaders)
    # model_ft = train_model(model, criterion, optimizer_ft, exp_lr_scheduler,
    #                      num_epochs, device, dataset_sizes, dataloaders)
    ######################################################################
    # 模型驗證
    mode = visualize_model(model_ft)
    # '''
    ...
    #
    model, criterion, optimizer, scheduler = part_transfer()
    model_conv = train_model(model, criterion, optimizer, scheduler, num_epochs,
                             device, dataset_sizes, dataloaders)
    ######################################################################
